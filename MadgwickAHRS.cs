...
namespace CoreLinkSys1.AHRS
{
    ...
    /// AHRS.Update(deg2rad(convertedGyroX), deg2rad(convertedGyroY), deg2rad(convertedGyroZ),convertedAccX, convertedAccY, convertedAccZ);
    /// </summary>
    /// <remarks>
    ...
    public class MadgwickAHRS
    {
        ...
        // 線形加速度（重力加速度ぬいたセンサ座標系での動的加速度）
        ///  float[] linearAcc = CalculateLinearAcceleration(AccX, AccY,AccZ,
        ///   AHRS.Quaternion[0], AHRS.Quaternion[1], AHRS.Quaternion[2], AHRS.Quaternion[3]);
        public static float[] CalculateLinearAcceleration(float accX, float accY, float accZ, float q0, float q1, float q2, float q3)
        {
            // 重力ベクトルを計算 (クォータニオンから)
            ...
            // 重力成分を除去
            float linearX = accX - gx;
            float linearY = accY - gy;
            float linearZ = accZ - gz;

            return new float[] { linearX, linearY, linearZ };
        }

        // 初期姿勢を基準(初期状態のクォータニオンが (1, 0, 0, 0)、つまり回転なし（= ワールド座標系と同じ）)としたグローバル加速度
        ///  float[] globalAcc = ConvertToGlobalFrame(linearAcc[0], linearAcc[1], linearAcc[2],
        ///            AHRS.Quaternion[0], AHRS.Quaternion[1], AHRS.Quaternion[2], AHRS.Quaternion[3]);
        public static float[] ConvertToGlobalFrame(float accX, float accY, float accZ,
                                   float q0, float q1, float q2, float q3)
        {
            // クォータニオンで回転させる（センサ座標系→グローバル座標系）
            float[] globalAcc = new float[3];

            // クォータニオンによる回転行列（方向余弦行列）を適用
            globalAcc[0] = (1 - 2 * q2 * q2 - 2 * q3 * q3) * accX +
                           (2 * q1 * q2 - 2 * q0 * q3) * accY +
                           (2 * q1 * q3 + 2 * q0 * q2) * accZ;

            globalAcc[1] = (2 * q1 * q2 + 2 * q0 * q3) * accX +
                           (1 - 2 * q1 * q1 - 2 * q3 * q3) * accY +
                           (2 * q2 * q3 - 2 * q0 * q1) * accZ;

            globalAcc[2] = (2 * q1 * q3 - 2 * q0 * q2) * accX +
                           (2 * q2 * q3 + 2 * q0 * q1) * accY +
                           (1 - 2 * q1 * q1 - 2 * q2 * q2) * accZ;

            return globalAcc;
        }


        //  var converted = ConvertSensorAxes(record.GyroX, record.GyroY, record.GyroZ, 
        // record.AccX, record.AccY, record.AccZ, 
        // record.MagX, record.MagY, record.MagZ);
        public static float[] ConvertSensorAxes(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz)
        {
            // ジャイロ
            float gx_t = gy;    // y
            float gy_t = -gz;   // -z
            ...
            return new float[] { gx_t, gy_t, gz_t, ax_t, ay_t, az_t, mx_t, my_t, mz_t };
        }
    }
}