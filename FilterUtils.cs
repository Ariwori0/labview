...
namespace CoreLinkSys1.Analysis
{
    public static class FilterUtils
    {
        ...
        public static List<double> ButterworthLowpassDouble(
    List<double> data, int fs, double cutoffHz, int order)
        {
            double nyq = 0.5 * fs;
            double wn = cutoffHz / nyq;
            wn = Math.Max(1e-6, Math.Min(0.999, wn)); // 正規化カットオフをクランプ

            double[] b, a;
            Signal.ButterworthLowpass(order, wn, out b, out a);

            var y = FiltFiltDouble(b, a, data.ToArray());
            return y.ToList();
        }

        private static double[] FiltFiltDouble(double[] b, double[] a, double[] x)
        {
            int padlen = 3 * Math.Max(a.Length, b.Length);
            if (x.Length <= padlen + 1) padlen = Math.Max(0, x.Length - 2);

            var xx = PadReflect(x, padlen);
            var y = ApplyFilterDouble(b, a, xx);

            Array.Reverse(y);
            y = ApplyFilterDouble(b, a, y);
            Array.Reverse(y);

            // パディングを外す
            return y.Skip(padlen).Take(x.Length).ToArray();
        }

        private static double[] PadReflect(double[] x, int pad)
        {
            if (pad <= 0) return (double[])x.Clone();
            int n = x.Length;
            var y = new double[n + 2 * pad];

            // 左側: 反射
            for (int i = 0; i < pad; i++)
                y[i] = 2 * x[0] - x[pad - i]; // 端から反転

            // 中央: 元データ
            Array.Copy(x, 0, y, pad, n);
        ...
        private static double[] ApplyFilterDouble(double[] b, double[] a, double[] x)
        {
            int n = x.Length;
            var y = new double[n];
            int na = a.Length;
